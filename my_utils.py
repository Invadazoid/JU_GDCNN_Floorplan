...
def get_args():
    ...
    parser.add_argument('--model', default = 'segnet',
                        help='fcn|deeplab|tiramisu|pspnet|unet|segnet')
    parser.add_argument('--dataset', default = 'camvid', 
                        help='camvid|cityscape|mapillary|bdd100k|synthia')
...
#            save_image(uncertainties[0],self.save_path+'/during_training/uncertainties/%03d.png'%(epoch))                
            ...
            if val_loss<best_val_loss:                          # Best Model
                ...
                torch.save(best_model,self.save_path+'/best_model.pth')
            # SAVE IMAGES
...
#        iou = iou/float(len(self.test_loader))
#        precision = precision/float(len(self.test_loader))
#        recall = recall/float(len(self.test_loader))
#        f1 = f1/float(len(self.test_loader))
        miou = iou.mean()
        mAP = precision.mean()
        mAR = recall.mean()
        mF1 = f1.mean()
